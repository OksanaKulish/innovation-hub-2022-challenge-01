...
```bash
test11$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
test11$ sam local start-api
test11$ curl http://localhost:3000/classify_digit
```
...
```bash
test11$ sam logs -n InferenceFunction --stack-name test11 --tail
```
...
```bash
aws cloudformation delete-stack --stack-name test11
```
